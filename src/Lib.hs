...
-- Prove that no position is played if the board is empty 
notPlayedEmpty :: CoordL row -> CoordL col -> Board rep -> Maybe (NotPlayed row col 'Empty) 
notPlayedEmpty row col (Board Empty) = Just (EmptyBoard row col (Board Empty)) 
notPlayedEmpty _ _ _                 = Nothing

-- To go further here we need full dependent types
notPlayedCons :: 
  CoordL row 
  -> CoordL col 
  -> Board ('Cons mark r c 'Empty) 
  -> Maybe (NotPlayed row col ('Cons mark r c 'Empty))
notPlayedCons row col board@(Board (Cons mark r c Empty)) = 
  do 
    emptyPrf <- notPlayedEmpty row col newBoard
    posPrf   <- _ (row,col) (r,c) -- type hole: no non-dependent function type Coord -> CoordL (coord :: Coord)
    return (PositionNotPlayed row col board posPrf emptyPrf)
...
-- What we don't have:
-- An implementation that computes new board types at runtime based on user input (full dependent types)
